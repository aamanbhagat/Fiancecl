...
export function generateIRRSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/irr', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/irr-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateIRRSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/irr')),
      }}
    />
  );
}
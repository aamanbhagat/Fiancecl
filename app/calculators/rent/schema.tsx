...
export function generateRentSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/rent', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/rent-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateRentSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/rent')),
      }}
    />
  );
}
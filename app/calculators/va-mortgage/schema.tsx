...
export function generateVaMortgageSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/va-mortgage', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/va-mortgage-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateVaMortgageSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/va-mortgage')),
      }}
    />
  );
}
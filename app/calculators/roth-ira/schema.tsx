...
export function generateRothIraSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/roth-ira', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/roth-ira-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateRothIraSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/roth-ira')),
      }}
    />
  );
}
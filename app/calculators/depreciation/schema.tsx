...
export function generateDepreciationSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/depreciation', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/depreciation-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateDepreciationSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/depreciation')),
      }}
    />
  );
}
...
export function generateSavingsSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/savings', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/savings-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateSavingsSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/savings')),
      }}
    />
  );
}
...
export function generateInterestRateSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/interest-rate', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/interest-rate-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateInterestRateSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/interest-rate')),
      }}
    />
  );
}
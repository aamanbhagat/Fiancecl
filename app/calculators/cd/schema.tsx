...
export function generateCDSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/cd', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/cd-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateCDSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/cd')),
      }}
    />
  );
}
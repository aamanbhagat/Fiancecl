...
export function generateInterestSchema(url: string) {
  ...
  const baseUrl = url.replace('/calculators/interest', '');
  ...
  return {
    ...
    '@graph': [
      // WebApplication schema for the calculator itself
      {
        ...
        'screenshot': {
          '@type': 'ImageObject',
          'url': `${baseUrl}/images/calculators/interest-screenshot.jpg`
        }
      },
  ...
  return (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: JSON.stringify(generateInterestSchema(process.env.NEXT_PUBLIC_SITE_URL + '/calculators/interest')),
      }}
    />
  );
}